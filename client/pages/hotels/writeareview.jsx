...
const Writereview = () => {
  ...
  var destination;
  destination = localStorage.getItem("hotel_name");
  var username;
  username = localStorage.getItem("email");
  const userReview = (e) => {
    e.preventDefault();
    Axios.post(`http://localhost:8000/api/hotels/postreview`, {
      stars: rating.stars,
      review: reviewText,
      ...
      .then((res) => {
        ...
        setTimeout(() => {
          router.push(`/hotels/${destination.toLowerCase().replace(/ /g, "-")}`);
        }, 1000);
      })
      .catch((err) => {
    ...
    <div>
      ...
      <div className="w-4/5 mx-auto">
        <div className="flex justify-end w-4/5 mx-auto">
          ...
          <Link href={`/hotels/${destination.toLowerCase().replace(/ /g, "-")}`}>
